    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['hachimiSys:feedback:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['hachimiSys:feedback:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['hachimiSys:feedback:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['hachimiSys:feedback:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listFeedback, getFeedback, delFeedback, addFeedback, updateFeedback } from "@/api/hachimiSys/feedback"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        applyId: [
          { required: true, message: "关联领养申请ID不能为空", trigger: "blur" }
        ],
      }
    }
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除领养后反馈信息编号为"' + feedbackIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('hachimiSys/feedback/export', {
        ...this.queryParams
