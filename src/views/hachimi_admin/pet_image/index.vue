    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['hachimi_admin:pet_image:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['hachimi_admin:pet_image:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['hachimi_admin:pet_image:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['hachimi_admin:pet_image:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="pet_imageList" @selection-change="handleSelectionChange">
...
<script>
import { listPet_image, getPet_image, delPet_image, addPet_image, updatePet_image } from "@/api/hachimi_admin/pet_image"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listPet_image(this.queryParams).then(response => {
        this.pet_imageList = response.rows
    ...
    handleUpdate(row) {
      ...
      getPet_image(imageId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.imageId != null) {
            updatePet_image(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addPet_image(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除宠物图片编号为"' + imageIds + '"的数据项？').then(function() {
        return delPet_image(imageIds)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('hachimi_admin/pet_image/export', {
        ...this.queryParams
      }, `pet_image_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
